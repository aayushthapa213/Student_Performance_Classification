...
model_path = r'ML_MODEL\knn_model.pkl'
...
def main():
    ...
    with col1:
        ...
        untrained_column = st.text_input(
            'Additional Information (not used in prediction)')
    ...
    Mjob = {'At Home': 0, 'Healthcare': 1, 'Other': 2,
            'Services': 3, 'Teacher': 4}.get(Mjob, 0)
    Fjob = {'At Home': 0, 'Healthcare': 1, 'Other': 2,
            'Services': 3, 'Teacher': 4}.get(Fjob, 0)
    reason = {'Close to Home': 0, 'Course Preference': 1,
              'Other': 2, 'School Reputation': 3}.get(reason, 0)
    ...
    with col2:
        ...
        if st.button('Predict'):
            ...
            st.write(
                f'Prediction for {student_name}: {"Pass" if prediction[0] == 1 else "Fail"}')
            ...
            sns.barplot(x=['Fail', 'Pass'], y=[1 - probability,
                        probability], ax=axes[0], palette=['red', 'green'])
            ...
            axes[2].pie([1 - probability, probability], labels=['Fail',
                        'Pass'], autopct='%1.1f%%', colors=['red', 'green'])
            ...
            if prediction[0] == 1:
                st.success(
                    f"{student_name} is likely to pass. Keep up the good work!")
            else:
                st.error(
                    f"{student_name} is likely to fail. Consider improving study habits and seeking additional help.")
